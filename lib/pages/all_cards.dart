...
//    _textEditingController.dispose();
  ...
  static Widget buildMembership(CardCounter counter, int index,
    ...
    CardInfo card = counter.getOneCard(index);
    return GestureDetector(
      onTap: () {
        Navigator.of(context).pushNamed("/cardinfo_membership", arguments: {
          "herotag": card.cardKey,
          "card": card
        });
      },
      child: Container(
          ...
          children: <Widget>[
            Container(
              ...
              child: Container(
                ...
                child: Text((card.useTimes % card.discountTimes).toString(),
                    style: TextStyle(
                      fontSize: 20.0,
                      color: Theme.of(context).primaryColor,
                    )),
  ...
  static List<Widget> _buildRewardPlace(int score, int rewardPoint, BuildContext context) {
    ...
    for (int i = 1; i < rewardPoint + 1; i++) {
      rewardList.add(Container(
        constraints: BoxConstraints.tightFor(
            height: 33.0,
            width: (MediaQuery.of(context).size.width - 64.0) / rewardPoint),
        decoration: BoxDecoration(
